...
public final class ParseEngine {
    ...
    public static void parse(ParserData data, int argc, String[] argv) {
        ...
        boolean[] consumed = new boolean[argc];
            ...
            if (Normalization.startsWith(effectiveArg, "--")) {
                ...
                case BARE_ROOT -> {
                    ...
                    if (!collected.hasValue && !hasAliasPresetTokens(aliasBinding)) {
                        ...
                        result.reportError(
                            effectiveArg,
                            "unknown value for option '" + effectiveArg + "'");
                    } else {
                        ...
                        invocation.valueTokens.addAll(
                            buildEffectiveValueTokens(aliasBinding, collected.parts));
    ...
    private static boolean isCollectableFollowOnValueToken(String value) {
    ...
    private static CollectedValues collectValueTokens(int optionIndex,
                                                      List<String> tokens,
                                                      boolean[] consumed,
                                                      boolean allowOptionLikeFirstValue) {
    ...
    private static void consumeIndex(boolean[] consumed, int index) {
        if (index >= 0 && index < consumed.length && !consumed[index]) {
            consumed[index] = true;
        }
    }

    private static boolean hasAliasPresetTokens(AliasBinding aliasBinding) {
        return aliasBinding != null && !aliasBinding.presetTokens.isEmpty();
    }

    private static List<String> buildEffectiveValueTokens(AliasBinding aliasBinding,
                                                          List<String> collectedParts) {
    ...
    private static int scheduleInvocation(CommandBinding binding,
        ...
        CollectedValues collected = collectValueTokens(
            index,
            tokens,
            consumed,
    ...
    private static void schedulePositionals(ParserData data,
                                            List<String> tokens,
                                            boolean[] consumed,
                                            List<Invocation> invocations) {
    ...
    private static List<String> buildParseTokens(int argc, String[] argv) {
        List<String> tokens = new ArrayList<>(argc);
        for (int index = 0; index < argc; ++index) {
            tokens.add(argv[index] == null ? "" : argv[index]);
        }
        return tokens;
    }

    private static void executeInvocations(List<Invocation> invocations,
                                           MutableParseOutcome result) {
