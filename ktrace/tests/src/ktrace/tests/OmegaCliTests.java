...
final class OmegaCliTests {
    private OmegaCliTests() {
    }
    ...
    private static void testBadSelector() throws Exception {
        TestSupport.ProcessResult result = TestSupport.runJava("ktrace.demo.omega.Main", "--trace", "*");
        Assertions.expect(result.exitCode() != 0, "invalid selector should fail");
        Assertions.expectContains(result.stderr(),
            "[error] [cli] option '--trace': Invalid trace selector: '*' (did you mean '.*'?)",
            "invalid selector should surface a helpful error");
    }

    private static void testExactSelectorWarning() throws Exception {
        TestSupport.ProcessResult result = TestSupport.runJava("ktrace.demo.omega.Main", "--trace", ".missing");
    ...
    private static void testWildcardSelectorWarning() throws Exception {
        TestSupport.ProcessResult result = TestSupport.runJava("ktrace.demo.omega.Main", "--trace", "missing.*");
    ...
    private static void testFilesOption() throws Exception {
        TestSupport.ProcessResult result =
            TestSupport.runJava("ktrace.demo.omega.Main", "--trace", ".app", "--trace-files");
    ...
    private static void testFunctionsOption() throws Exception {
        TestSupport.ProcessResult result =
            TestSupport.runJava("ktrace.demo.omega.Main", "--trace", ".app", "--trace-functions");
    ...
    private static void testWildcardDepth3() throws Exception {
        TestSupport.ProcessResult result =
            TestSupport.runJava("ktrace.demo.omega.Main", "--trace", "*.*.*.*");
        Assertions.expectEquals(result.exitCode(), 0, "depth3 wildcard should succeed");
        Assertions.expectContains(result.stdout(), "[omega] [app] omega initialized local trace channels",
            "omega app trace should be visible");
        Assertions.expectContains(result.stdout(), "omega trace test on channel 'deep.branch.leaf'",
            "deep leaf trace should be visible");
        Assertions.expectContains(result.stdout(), "beta trace test on channel 'io'",
            "imported beta trace should be visible");
        Assertions.expectContains(result.stdout(), "gamma trace test on channel 'physics'",
            "imported gamma trace should be visible");
    }

    private static void testBraceSelector() throws Exception {
        TestSupport.ProcessResult result =
            TestSupport.runJava("ktrace.demo.omega.Main", "--trace", "*.{net,io}");
